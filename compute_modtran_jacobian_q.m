function [] = compute_modtran_jacobian_q(date_str, profile, saveFileName)
    % Computes Jacobian matrix for log q perturbations: ∂R / ∂ln(q)
    perturbation_factor = 1.1;  % +10% perturbation (logarithmic kernel)
    ...
    for level = 1:n_levels
        ...
        perturbed_profile.q(level) = profile.q(level) * perturbation_factor;

        level_modroot = sprintf('jac_qpert_%s_level_%d', date_str, level);
        ...
        jacobian_matrix(:, level) = delta_rad / log(perturbation_factor);  % Log kernel

        perturbed_spectra{level} = perturbed_result;
        actual_perturbation(level) = log(perturbation_factor);
    ...
    jacobian_info.method = 'log10_q_perturb';
    jacobian_info.perturbation_log = log(perturbation_factor);
    jacobian_info.actual_perturbations = actual_perturbation;

    save(saveFileName, 'jacobian_info', '-v7.3');
    disp(['Saved q Jacobian to: ', saveFileName]);
end

function [] = run_multiple_lblrtm_retrievals(datestring,n_recalc_jacob)
    ...
    if nargin < 2
        n_recalc_jacob = 2; % jacobian recalculation
    end
    ...
    output_dir = fullfile('output_retrievals_lblrtm/38levels', datestring);
    ...
    for i = 1:20
        ...
        elseif i <= n_recalc_jacob
            ...
            disp(['Computing LBLRTM Jacobians for iteration ', num2str(i), '...']);
            tic
            [wv_aj, K_t, K_q] = lblrtm_aeri_aj_clearsky(v1, v2, ...
                profile_input.z, profile_input.p, profile_input.t, profile_input.t(1), profile_input.q, ...
                profile_input.co2, profile_input.o3, n2o, co, profile_input.ch4, ...
                ccl4, f11, f12, zenith, atmos_id, altitude_toa);
            toc

            % Subset and convert to RU
            [~, index_aj_begin] = min(abs(wv_aj - wv_begin));
            [~, index_aj_end]   = min(abs(wv_aj - wv_end));
            ...
            abs(nanmean(wv_aj_sub - AERI_wnum_adj')) > 0.01
                error('LBLRTM output wavenumbers not aligned with AERI wavenumbers.');
            end

            K_t = K_t(index_aj_begin:index_aj_end, :) .* 1e7;  % RU/K
            K_q = K_q(index_aj_begin:index_aj_end, :) .* 1e7;  % RU/log(ppmv)
            K_t(isnan(K_t)) = min(abs(K_t(:)));
            K_q(isnan(K_q)) = min(abs(K_q(:)));

            % Save for reuse
            save(jacobian_path_t, 'K_t');
            save(jacobian_path_q, 'K_q');
            save(jacobian_path_wv, 'wv_aj');
        ...
        % FORWARD SIMULATION
        if i == 1
            disp('Computing LBLRTM forward simulation...');
            tic
            [~, F] = lblrtm_aeri_clearsky(v1, v2, profile_input.z, profile_input.p, ...
                profile_input.t, profile_input.t(1), profile_input.q, ...
                profile_input.co2, profile_input.o3, n2o, co, ...
                profile_input.ch4, ccl4, f11, f12, zenith, atmos_id, altitude_toa);
            toc
        
            % Convert and subset to AERI grid
            [~, index_aj_begin] = min(abs(wv_aj - wv_begin));
            [~, index_aj_end]   = min(abs(wv_aj - wv_end));
            F = (F(index_aj_begin:index_aj_end))' .* 1e7;
